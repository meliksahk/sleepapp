...
void main() {
  ...
  group('ekrana özgü metin (onlineMessage)', () {
    Future<void> pumpWith(WidgetTester tester) => tester.pumpWidget(
      MaterialApp(
        localizationsDelegates: AppL10n.localizationsDelegates,
        supportedLocales: AppL10n.supportedLocales,
        theme: buildNoctaDarkTheme(),
        home: Scaffold(
          body: NetworkErrorView(
            retryKey: const Key('retry'),
            onlineMessage: 'Your shares could not be loaded.',
            onRetry: () {},
          ),
        ),
      ),
    ...
    testWidgets('ağ AÇIKKEN ekranın kendi metni + yeniden dene', (tester) async {
      FlavorConfig.current = const FlavorConfig(
        flavor: Flavor.dev,
        name: 'DEV',
        apiBaseUrl: 'http://localhost:3001',
      ...
      expect(find.text('Your shares could not be loaded.'), findsOneWidget);
      expect(find.byKey(const Key('retry')), findsOneWidget);
    });

    testWidgets('ağ KAPALIYKEN ekranın metni DEĞİL dürüst metin, düğme yok', (
      tester,
    ) async {
      FlavorConfig.current = const FlavorConfig(
        flavor: Flavor.prod,
      ...
      await pumpWith(tester);
      // "Paylaşımların yüklenemedi" + yeniden dene, sunucusu olmayan bir
      // yapıda yanıltıcıydı: deneme hiçbir zaman işlemez.
      expect(find.text('Your shares could not be loaded.'), findsNothing);
      expect(find.textContaining('needs an account connection'), findsOneWidget);
      expect(find.byKey(const Key('retry')), findsNothing);
    });
  });

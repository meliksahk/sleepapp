...
void main() {
  testWidgets('detay: başlık + preset sayısı + preview göstergesi', (tester) async {
    await _pump(tester, 'deep-ocean', [
      soundscapeDetailProvider('deep-ocean').overrideWith(
        (ref) async => SoundscapeDetail(
          ...
          presets: [const Preset(archetypeSlug: 'deep-ocean', mixerState: null)],
  ...
  testWidgets('hata → retry', (tester) async {
    await _pump(tester, 'x', [
      soundscapeDetailProvider('x').overrideWith((ref) async => throw Exception('ağ')),
    ]);

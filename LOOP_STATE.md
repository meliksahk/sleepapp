...
## 🚧 İlerleme: **%80** — formül 80.20 (tablodan, #137 kuralı)
...
- **#215 (tam demo soundscape — kullanıcının bulduğu kök sebep):** Kullanıcı "ses kombinasyonları yarım yamalak çalışıyor, mikserde bu kombinasyon ayarı yok" dedi. **Haklıydı ve kök sebebi ben gözden kaçırmıştım:** motor #213'te pad/fire/rain/waves öğrendi ama seed'deki 6 tarif **#210'da**, yani o kaynaklar HENÜZ YOKKEN yazılmıştı; hepsi yalnızca white/pink/brown kullanıyordu. Yani yetenek vardı, onu kullanan İÇERİK yoktu — kullanıcı mikseri açınca sadece gürültü görüyordu. `Hearth & Static` eklendi: pad 0.34 (melodik gövde) + white 0.30 (maskeleyici) + fire 0.26 (efekt), toplam 0.90 < 1.0 (kırpma payı korunuyor). Doğrulama: seed idempotent koştu · API üç katmanı döndürdü · emülatörde kütüphanede en üstte göründü · player üç sürgüyü ayrı ayrı gösterdi ("Warm pad %34 / White noise %30 / Crackling fire %26") · Çal → `dumpsys audio` **3 adet AudioTrack state:started**. CI iki iş de yeşil, merge edildi. PR #215.
  - **DERS:** yeni bir motor yeteneği eklendiğinde İÇERİĞİN de güncellenmesi gerekiyor; aksi hâlde yetenek görünmez kalıyor ve ürün "yarım" hissettiriyor. #213'te bunu "kapsanmayan" diye not etmiştim ama sıradaki işe almamıştım.
  - **Kullanıcı bu turda tam durum raporu istedi** ve raporu okuyup nasıl ilerleyeceğine KENDİSİ karar vereceğini söyledi.

- **#214 (ambiyans + player + global sayaç + açılış anı):** Dört parça, tek PR. **(1) Ambient arka plan:** `CustomPainter` + Ticker (GLSL shader REDDEDİLDİ — `FragmentProgram.fromAsset` widget testinde yüklenmiyor, yani "arka planda durdu mu / faz sesle aynı mı" CI'da KANITLANAMAZDI). **FFT YOK:** sesi biz ürettiğimiz için faz zaten biliniyor; `ambient_phase.dart` ses sabitlerini KOPYALAMADAN import ediyor, ölçülen ses↔görsel korelasyon **r = 0.9281**. 12 fps (60 değil — en hızlı büyüklük 10 sn'de bir çevrim, kare başına ~3°), kare 48–69 µs, arka planda **0 kare**. **(2) Global uyku sayacı:** kabuk katmanında (çevrimdışı bandıyla aynı yer), saniyede bir güncelleniyor ama denetim ölçtü: **ekran ağacı boyaması 0** (Navigator'ın RepaintBoundary'si arkasında). **(3) Player:** mikser ekranı hero + kontrol sayfası olarak yeniden kuruldu, arkasında ambiyans. **(4) Açılış anı:** dalgalanan hilal ay, **sesle senkron** — `nocta_signature.dart`'ın zarf sabitleri public yapılıp paylaşıldı, ay parlaklığı auranın raised-cosine zarfı, yıldızlar tam olarak sesin parıltı taneleri anında doğuyor (mutasyon: zarfı sabitle → 5 test kırılıyor). Silüet dalgalandırma REDDEDİLDİ (görünür genlikte hilal amorf bir damlaya dönüyor, marka tanınırlığı tam açılış anında kayboluyor); onun yerine ışık ayın içinden geçiyor. Native splash → Flutter dikişi: her iki zemin de `#0A0E1A`, testle kilitli.
  - **DENETİM KÖK SEBEPLERİ BULDU (yama değil):** dikiş çizgisi = hero scrim'i 0'a sönerken kontrol paneli 0.72'den başlıyordu, **0.72'lik alfa basamağı** ekranda düz çizgiydi → tek monoton profil. Tek slider = hero'nun sabit `flex:3`'ü ekranın %37.5'ini yiyordu → içerik-boyutlu hero, görünen katman 390×844'te **2→4**, kaydırınca 7/7. Parlaklık sıçraması = çizim duruyordu ama SAAT durmuyordu → saat de donduruldu, **%146.8 → normal kare adımı**.
  - **KENDİ HATAMI DENETİM YAKALADI:** yazdığım kontrast testi üretimin bir KOPYASINI ölçüyordu ve kopya ayrışmıştı — test yeşildi, ölçtüğü şey ekranda yoktu. Üstelik eski 0.72 alfası gerçekte **4.45** veriyor (AA eşiği 4.5'in ALTINDA), yani "AA geçiyor" iddiam geçersizdi. Test artık üretim sabitlerini import ediyor; ölçülen **5.38–6.01**.
  - **Ayrıca düzeltildi:** hareketi-azalt açıkken üst sınır hiç uygulanmıyordu (ticker çalışmadığı için) → **sonsuz splash mümkündü**; ticker'dan bağımsız zamanlayıcı + regresyon testi.
  - **Doğrulandı:** mobil **651 test**, analyze temiz, turbo 19/19, tüm uyum kapıları yeşil. Emülatörde prod APK: ay ~0.9 sn'de ekranda, ~1.7 sn'de ana ekran, renk sıçraması yok.
  - **Kapsanmayan:** her açılışta **1.44 sn** zorunlu bekleme (1.1 hold + 0.32 geçiş) — bilinçli ama kullanıcı onayı alınmadı. Yazı ölçeği 1.6/2.0'da 320×568'de taşma (eski düzenden ~6 px daha kötü). Ses playhead'ine faz kilidi hâlâ yok (periyot kilitli, t=0 ofseti ölçülmedi). Ses KULAKLA dinlenmedi.
  - **KULLANICI SORUSU CEVAPLANDI:** "neden hâlâ çevrimdışı diyor" — kurduğum APK **prod flavor**'dı (`api.nocta.app` = ölü) ve `adb reverse`'ü doğrulama kapısı için bilerek kaldırmıştım. Dev flavor + reverse ile bant kayboldu, lokal API'ye bağlanıldı.

- **#213 (meditatif ses katmanları — dalga, ateş, yağmur, melodik pad):** Kullanıcı isteği: "meditatif müzik ve dalga/ateş gibi sesler beyaz-kahverengi gürültüyle birleşebilmeli, hem mikserde hem soundscape mantığında"; YouTube'dan çekmeyi de önerdi. **YouTube REDDEDİLDİ ve gerekçesi `docs/04 §1.2.0`'a kalıcı olarak yazıldı:** Google Play, YouTube içeriği indiren uygulamaları açıkça yasaklıyor (bilinen ret sebebi, kullanıcının "mağazaya çıkacak uygulama" hedefini doğrudan öldürür) + müzik telifli + #212'de kurulan backend'siz mimariyi bozar. Yerine SENTEZ: motorun bildiği kaynak 3 → 7 (`NoiseType` → `LayerSource`). Ürün avantajı teselli değil: sentez **hiç döngüye girmez** (örneklenmiş uyku sesindeki en yaygın şikâyet duyulabilir loop noktasıdır), bant genişliği/depolama sıfır, internetsiz çalışır. v2'de gerçek kayıt istenirse yol CC0/ısmarlama + "asset katmanı" tipi; bugünkü karar o kapıyı kapatmıyor.
  - **AJANIN KENDİ BULDUĞU TUZAK (benim uyarımın ötesinde):** pad döngü-periyodik olunca kuyruğu ile başı BİREBİR aynı; `renderSeamlessLoop`'un eşit-güç crossfade'i (korelasyonSUZ sinyal için tasarlanmış) onları `sin+cos ≤ √2` ile toplayıp HER DÖNGÜ BAŞINDA 50 ms **+3 dB kabarma** üretecekti. Çözüm: katman başına `isLoopPeriodic` kararı — kilitli katman ham kopyalanır. Denetçi bunu bağımsız doğruladı: pad ham dikişi 4 seed'de de TAM SIFIR.
  - **BENİM KOYDUĞUM ŞART TUTTU:** her modülasyon periyodu 30 sn'lik döngüyü tam bölüyor (dalga 10 sn, pad nefesi 15 sn, partial'lar 30 sn'de tam sayıda periyot). Denetçi zarf sürekliliğini ölçtü: hiçbir kaynakta dikiş, buffer içindeki normal dalgalanmanın dışına çıkmıyor.
  - **ÜÇ MERCEKLİ DENETİMDEN DÜZELTİLENLER:** (1) **`renderMix` render süresini döngü periyodu sanıyordu** → mix-to-video export'u (viral kanca #3) kullanıcının DUYDUĞUNDAN farklı bir dalga karakteri paylaşıyordu (çalarken 10 sn kabarma, videoda 7.5 sn — ölçüldü). `loopSeconds` açık parametre oldu, export çalma yolunun periyodunu geçiyor, regresyon testi kilitledi. (2) **Pad render'ı UI isolate'ini bloke ediyordu** (pad tek başına 189 ms, yedi katman 285 ms) — kod tabanının kendi kuralı (`signature_player.dart` `compute()` kullanıyor) pad'e uygulanmamıştı; render isolate'e taşındı, testler için enjekte edilebilir yapıldı (`playerFactory` deseni). (3) **Dikiş testi tek örneklik tıkı GÖREMİYORDU** — denetçi dikişe %20 genlikli pop enjekte etti, test yine geçti; örnek düzeyi dikiş testi + negatif kontrol eklendi. (4) `mix_player.dart`'ın "katmanlar gürültü, tonal katman eklenirse varsayım çöker" yorumu pad tonal olduğu hâlde güncellenmemişti — dürüstçe düzeltildi (tek pad sorun değil, iki pad ölçülmedi).
  - **DOĞRULANDI:** mobil 524 → **555 test**, api içerik 47, admin 145, analyze temiz, `check-layer-source-drift` (mobil/sunucu/panel sıra dahil senkron) CI'a bağlandı. **PROD APK + emülatör:** mikserde "Ocean waves" sürgüsü göründü, Çal'a basıldı → `dumpsys audio`: **7 adet `AudioTrack state:started`** (yedi katmanın hepsi). Ölçümler: waves tepe 0.5555/RMS 0.0872, fire 0.4952/0.1069, rain 0.4436/0.1079, pad 0.3024/0.0816; yazılı tepe sınırları (0.96/0.80/0.64/0.491) hepsi 1.0 altında; modülasyon enerjisi @10 sn = 0.749, diğer periyotlarda ~0.01.
  - **AÇIK BIRAKILAN (görev #15):** mikser sürgülerinde toplam kazanç sınırı YOK. 7 katman @1.0 → örneklerin %14.1'i kırpılıyor. **Bu #213'ten ÖNCE de vardı** (3 gürültü @1.0 → %12.0) ve varsayılan ayarda kırpma yok (tepe 0.703), ama tavan 3.0→5.89 ve sürgü 3→7 olduğu için maruziyet büyüdü. Asıl sorun: `MixPlayer` Dart `Mixer`'ını kullanmıyor, toplama OS mikserinde oluyor ve orada clamp yok. Önerim master limitleyici ama mikserin HİSSİNİ değiştirir → kullanıcı kararı.
  - **SES KULAKLA DİNLENMEDİ.** Ne ben ne ajanlar. "Güzel/doğal duyuluyor" iddiası YOK; yalnızca ölçülen sayılar raporlandı (§1.1: estetik yargı kullanıcıya ait).

- **#212 (arketip testi ON-DEVICE — backend'siz APK'da kanca #1 uçtan uca çalışıyor):** **Müdür kararı, ve haklıydı: bir kör noktayı yakaladı.** Doğruladığı şey şu — `api.nocta.app` DNS'te ölü, `main_prod.dart` oraya bakıyor, yani kullanıcının kuracağı PROD APK'da arketip testi/kütüphane/haftalık içerik/premium hepsi ölüydü. Benim tüm emülatör doğrulamalarım DEV flavor + `adb reverse` ile lokal API'ye bakıyordu ve bu ölümü tamamen maskeliyordu. **Kararlarım:** (K1) matris ELLE kopyalanmadı — `tooling/gen-archetype-matrix.mjs` API domain'ini `node:vm` içinde ÇALIŞTIRIP (regex ayrıştırma değil) tek JSON üretiyor → `assets/archetype/matrix.json`; `check-archetype-drift.mjs` bayt bayt karşılaştırıp CI'da kapı oluyor (repo deseni: gen:api-types, gen:tokens, check:env-example). Gerekçe: elle kopyalama iki uygulamanın SESSİZCE ayrışması demekti — aynı cevaplar farklı sonuç. (K2) puanlama Dart'a portlandı, tie-break TS ile birebir. (K3) kalıcılık prefs (tek küçük kayıt; drift ağır kalırdı). (K4) sunucu opsiyonel/best-effort, hata gösterilmiyor. (K5) `NoctaApiClient`'a 5 sn timeout (hiç yoktu; ölü backend'de her ekran OS varsayılanına kadar donuyordu). **Eşdeğerlik kanıtı:** denetim ajanı 4^6=4096 tam kombinasyon + 8 kenar durumu iki tarafta koşturdu → `4104 vaka, 1561 beraberlik, 0 uyuşmazlık`. **Üç mercekli adversaryal denetim beş orta bulgu çıkardı, dördü düzeltildi:** (1) `share()` sunucu-öncelikliydi → ölü backend'de paylaş butonu 5 sn donuyordu; aynı serviste `latest()` 0 ms iken `share()` 5024 ms ölçüldü — bu iterasyonun TAM iddiasını en kritik yolda çürütüyordu, yerel-öncelikli yapıldı; (2) `latest()` yerel boşken 5 sn blokluyordu (testi hiç yapmamış kullanıcının ana ekranı, her açılış) → keşif çağrılarına 1.2 sn bütçe; (3) yerel depo yalnızca `FormatException` yakalıyordu, oysa yorumu "eski şema tolere edilir" diyordu — YANLIŞTI: geçerli JSON/yanlış şekil `TypeError` atıp ekrana sızıyordu ve depo artık TEK doğruluk kaynağı olduğu için tek bozuk kayıt özelliği o cihazda KİLİTLİYORDU (4 vaka testle kilitlendi, log'lu geniş catch); (4) bayat yorumlar. **Bedava yolcular:** mikser başlığı artık açılan sesin adını gösteriyor; "Soundscape'lere göz at" kelime bölünmesi düzeltildi (kök sebep ölçüldü: 360dp'de metin alanı 122px, kelime 126.5px). mobil 491 → **524 test**, api 21 test, tüm kapılar yeşil.
  - **DOĞRULAMA KAPISI GEÇİLDİ (müdürün pazarlıksız şartı):** `flutter build apk --release -t lib/main_prod.dart` → `adb reverse --remove-all` (lokal API'ye kaçış yolu YOK) → temiz kurulum. Sonuç: uygulama açıldı (dürüst çevrimdışı bandı), test **gömülü matristen** yüklendi, 6 soru cihazda puanlandı, **"Deep Ocean" sonucu ~3 sn'de** geldi, paylaşım sayfası **~2.6 sn'de** açıldı (kart görseli + link hazır; 5 sn'lik blok gitti), uygulama öldürülüp yeniden açıldığında **sonuç ana ekranda duruyordu**. 9 ekran görüntüsüyle kanıtlı.
  - **YENİ KEŞİF — kullanıcı kararı gerekiyor (görev #14):** `nocta.app` BİZİM DEĞİL. Kendim doğruladım: kök alan A kaydı 76.76.21.21 (Vercel, DEPLOYMENT_NOT_FOUND), `api.nocta.app` → herokudns.com CNAME. Yani prod APK cihaz parmak izi + token isteklerini bize ait OLMAYAN bir hosta gönderiyor (§6 riski) ve paylaşılan kart linki başkasının alanına gidiyor — bu iterasyonda ağsız paylaşım çalışır hale geldiği için risk artık VARSAYILAN yol. Emülatörde canlı görüldü: `https://nocta.app/a/deep-ocean`.
  - **Kapsanmayan:** soundscape kütüphanesi prod APK'da hâlâ sunucuya bağlı (müdür bunu bir SONRAKİ iterasyon olarak sıraladı: 6 soundscape + varsayılan tarifler APK'ya gömülü). Kalıcı yeniden deneme kuyruğu yok. Yerel↔sunucu birleştirme yok. `fetchContent()` ölü kod.

- **#211 (soundscape kütüphanesi artık SES ÇIKARIYOR):** #210'un açık bıraktığı tek parça. Kullanıcı kütüphaneyi görüyordu, bir sese tıklıyordu, başlık ve preset sayısını okuyordu — **ama ses çıkmıyordu.** Parçaların hepsi hazırdı (mixSpec parse ediliyor, `Preset.toMixSpec()` var, `MixerController` zaten `MixSpec` parametresi alıyor), arada tel yoktu. **Ayrı oynatıcı YAZILMADI:** detaydaki "Bu sesi çal" mikseri o sesin kendi tarifiyle açıyor (`/mixer?soundscape=<slug>`; query seçildi çünkü go_router `extra` derin linkte ve yeniden başlatmada kaybolur). Böylece kullanıcı sesi duyuyor VE hemen slider'larla kendi mix'ine dönüştürüp videoya çevirebiliyor (viral kanca #3 bedavaya bağlandı). `resolveSoundscapeMix` çözüm sırası: soundscape'in tarifi → ilk geçerli preset (`Preset.toMixSpec()` ölü kodu böylece canlandı) → varsayılan. **Offline-first (§3.1) korundu:** bilinmeyen slug, ağ hatası ve _hiç dönmeyen istek_ (API istemcisinde timeout YOK — 3 sn bütçe eklendi) yollarının üçünde de mikser AÇILIR ve ÇALAR, hata ekranına düşmez. **`limitTotalGain` eklendi:** sunucu katman başına 0..1 doğruluyor ama TOPLAMI doğrulamıyor (3 katman × 1.0 geçerli bir tarif); MixPlayer katmanları ayrı player'larda çalıp OS mikserinde topladığı için toplam 1.0'ı aşarsa OS seviyesinde kırpma olurdu — tarif oranları korunarak ölçekleniyor. Otomatik çalma YOK (uyku uygulaması; kullanıcı iki kez eylem yapar).
  - **ADVERSARYAL DENETİM İKİ GERÇEK BULGU ÇIKARDI, İKİSİ DE DÜZELTİLDİ VE MUTASYONLA KANITLANDI:** (1) **KRİTİK — router bağlantısı testsizdi:** `router.dart`'taki `queryParameters['soundscape']` ifadesi `null` yapıldığında (özellik tamamen ölür) 487 testin TAMAMI yine geçiyordu; zincirin iki ucu testliydi ama URL'den slug okuma halkası hiç koşturulmuyordu. Gerçek `appRouter`'ı koşturan `test/app/mixer_deeplink_route_test.dart` yazıldı — aynı mutasyon artık İKİ testi birden kırıyor. (2) **ORTA — dürüstlük dipnotu sessizce siliniyordu:** 3 sn bütçe dolup varsayılanla açıldıktan SONRA ağ yanıtı geç gelirse `usedFallback` false oluyor ve "tarif yüklenemedi" dipnotu ekrandan kayboluyordu — ama çalan ses hâlâ varsayılan (spec initState'te dondurulur, bilinçli). Yani kullanıcı seçtiği sesi duymuyor ve bunu söyleyen tek işaret de yok oluyordu; üç denetim merceği de bunu bağımsız yakaladı. Mandal (latch) eklendi, mandalı kaldırma mutasyonu ilgili testi kırıyor.
  - **CİHAZDA DOĞRULANDI (ajanın yapamadığı adım):** emulator-5554'te dev APK ile kütüphane → Deep Ocean Hush → "Bu sesi çal" → mikser **tam o tarifle** açıldı (Kahverengi 0.75 + Pembe 0.18; varsayılanın üçüncü katmanı yok) → Çal → `dumpsys audio`: `com.nocta.nocta`'dan **İKİ** `AudioTrack state:started usage=USAGE_MEDIA sampleRate=48000` (iki katman = tarif birebir). Kütüphane ayrıca arketip yakınlığına göre sıralanıyor (Deep Ocean Hush başta) ve haftalık yayın kartı seed'den Türkçe geliyor. mobil 472 → **491 test** yeşil, analyze temiz.
  - **Kapsanmayan:** ses KULAKLA dinlenmedi (sistem "çalıyor" diyor, "güzel duyuluyor" iddiası yapılamaz — §1.1 gereği yargı kullanıcıda). Tarif önbelleği yok → gerçek offline'da mikser hep varsayılana düşer ("internetsiz TAM çalışır" sözü şu an "internetsiz VARSAYILAN tarifle çalışır" seviyesinde). API istemcisinde hâlâ timeout yok (3 sn bütçe semptomu sarıyor, kökü değil). Mikser başlığı jenerik "Mikser" — hangi ses açıldığı yazmıyor. Kullanıcı slider'ları elle yukarı çekerse toplam kazanç yine 1.0'ı aşabilir (mevcut davranış, regresyon değil). Ana ekranda "Soundscape'le / re göz at" kelime bölünmesi kusurlu.

- **#210 (lokal tam yığın: TR web/admin + seed + arketip tasarımı + CORS):** Kullanıcı "lokalde hepsi haberleşsin, her özellik çalışsın, girişleri seedle, web+admin'i çalıştır, soundscape örnekleri ekle" dedi. **Yolda GERÇEK BİR HATA bulundu: API'de CORS hiç yapılandırılmamıştı.** Tanıtım sitesinin public uçları (`/v1/archetype/web/questions`, `/v1/waitlist`) tarayıcıdan çağrılmak üzere tasarlanmış ama tarayıcı onlara hiç ulaşamıyordu — web arketip testi `net::ERR_FAILED` alıp "Sorular yüklenemedi" gösteriyordu, yani **o özellik hiç çalışmamış**; üretimde de kırılacaktı (nocta.app → api.nocta.app farklı kaynak). İzin listesi env'den (`CORS_ORIGINS`), joker DEĞİL, reddedilen kaynak loglanıyor; 8 test. **Seed:** 6 soundscape + 5 preset + haftalık yayın; "soundscape bir ses DOSYASI değil, bir ses TARİFİ" sözleşmesi yorumlara yazıldı; `/v1/content/weekly` 404'ten çıktı; `pnpm db:seed` (Windows'ta psql'siz docker exec'e düşer, ON_ERROR_STOP=1). **Arketip ekranı yeniden tasarlandı:** markalı AppBar, NCard'lı sorular, ilerleme göstergesi, yeni `NSelectableOption` (seçili hal renk TEK taşıyıcı değil: kenarlık + tik), `NErrorState`, sonuçta arketip gradyanı; tek kaydırma düzeni korundu (PageView gating testini kırardı). **Admin i18n tamamlandı:** sözlük 29→201 anahtar, 19 dosyadaki sabit metin taşındı, sunucu bileşenlerinde `translate(getLocale())` (`'use client'`a çevrilmedi → JS bütçesi korundu), Intl tarih/sayı/yüzde, LocaleSwitcher /login ve /security'ye de eklendi. **Web TR:** `/tr/` alt dizini (kök EN'e dokunulmadı → mevcut testler ve bundle kapısı sağlam), hreflang + x-default, TR sitemap; ayrıca `lang="en"` altında render edilen KAÇAK Türkçe metinler ("Sonucu gör", "Katıl") temizlendi. 8 ajanlı iş akışı (4 uygulama + 4 adversaryal denetim) kullanıldı; denetimden çıkan 3 bulgu düzeltildi: og:url TÜM TR sayfalarda EN ana sayfayı gösteriyordu (sosyal paylaşım yanlış nesnede birleşiyordu), rollout yüzdesi ondalık yuvarlanıyordu (%12,5→%13; `formatPercent`'e `fractionDigits` eklendi — metrikler 0, elle girilen rollout 1), "Gövde"→"Mesaj". **Flake düzeltmesi:** `throttler.e2e` paylaşılan Redis kotasına bağlıydı ve arka arkaya iki koşuda biri geçip biri kalıyordu; sayaçlar artık her testten önce sıfırlanıyor — tam paket arka arkaya İKİ KEZ 518/518. api 518 · admin 145 · web 78 · mobil 472 test yeşil. **Lokal uçtan uca kanıtlandı:** admin :3002 → kendi vekili → API :3011 → DB (giriş çerezi yazıldı, 6 soundscape panelde göründü); web :3003/tr/test GERÇEK TARAYICIDA API'den Türkçe soruları çekti. PR #210.
  - **CI'da İKİ KEZ düştüm, ikisi de kendi eksikliğim:** (1) test ekleyip `typecheck`'i tekrar koşmadım (vitest tip denetimi yapmaz), (2) env şemasına değişken ekleyip `.env.example`'ı güncellemedim — repo tam da bunun için bir kapı tutuyor ve yakaladı. Sonrasında CI'ın TÜM adımları lokalde tek tek koşturuldu.
  - **Kapsanmayan:** soundscape'ler kütüphanede GÖRÜNÜYOR ama ÇALMIYOR (mobil detay ekranı tarifi motora vermiyor) — "her özellik çalışsın" isteğinin açık kalan tek parçası, sıradaki iterasyon. Ses tarifleri kulakla DİNLENMEDİ. Blog TR'ye çevrilmedi. `db/seed.sql` sağlık-iddiası taramasının dışında (.sql taranmıyor).

- **#209 (arketip testi TÜRKÇE — sunucu içeriği dile uyuyor):** Kullanıcı "uyku kimliğini keşfet neden çalışmıyor" diye sordu; **lokal deploy** ile (API :3011 — 3000/3001 CRM'de dolu, `adb reverse tcp:3001 tcp:3011`, manifest gevşetilmedi) çalıştırınca gerçek kusur görüldü: arayüz Türkçe ama **sorular ve sonuç anlatımı İngilizce** — hiçbir istek `Accept-Language` göndermiyordu. `shared/locale.ts` (RFC 9110: q değerleri, alt etiket soyma, bilinmeyen dil → EN, istek asla kırılmaz) + `archetype-i18n.ts` (TR matris) + TR tanıtım metni. **Tasarım kuralı: yalnızca metin çevrilir; id ve arketip eşlemesi dokunulmaz → skorlama dilden bağımsız (test edilen değişmez).** İsim ("Deep Ocean") çevrilmez — paylaşım kartı/`/a/{slug}` tanınırlığı bölünmesin. Mobil: dil **istek anında** çözülür. **Yol boyunca kendi eklediğim İKİ regresyon yakalandı ve düzeltildi:** (1) dil `watch` edilince `apiClientProvider` yeniden kurulup eski `http.Client` kapanıyordu, `AuthController` onu `read` ile tuttuğu için dil değiştiren kullanıcının TÜM istekleri ölüyordu (emülatörde kanıtlandı: sunucuya tek istek ulaşmadı); (2) içerik açılışta önbelleğe alındığı için sorular Türkçe, sonuç açıklaması İngilizce kalıyordu. İkisi de regresyon-kilit testiyle kapatıldı. api 5 suite/23 test + 14 yeni domain testi (sağlık iddiası taraması TR metinlerde dahil), mobil 463/463 yeşil, emülatörde uçtan uca doğrulandı. **Ayrıca kendi yanlış teşhisimi düzelttim:** "seçim geri bildirimi yok" demiştim — kodu okuyunca ve doğru ekran görüntüsünü alınca **çalıştığı** görüldü; kaydırıp geçtiğim için kadraja girmemişti. PR #209.
  - **Kapsanmayan:** soundscape/haftalık içerik hâlâ yalnızca sunucudaki dilde (DB'den geliyor, çeviri DB şeması gerektirir — ayrı iş). Arketip test ekranının **tasarımı** hâlâ çıplak liste (kart yok, ilerleme göstergesi yok) ve hata durumu çıplak yenile ikonu (#202 bu ekranı atlamıştı).
...
- **Çok-ajanlı sonik-kimlik paneli + jüri:** 3 konsept (statik pad / melodik motif / hibrit) →
  kazanan **hibrit** (27p): sabit doku + seyrek parıltılar. Jingle gece 23:00'te yanlış, düz pad
  jenerik; ezgisiz ama tanınabilir olan tek tasarım. Pad=ufuk, parıltı=yıldız.
- **Yapıldı:** `nocta_signature.dart` — 5 partial pad (f0=130.81 C3, ağırlık toplamı tam 1.00,
  sabit-Hz detune'lar birbirinin katı değil → tekrar etmez), smoothstep tilt, f0 katlarında
  parıltılar (disonans imkânsız), pinkNoise+DcBlocker hava, yükseltilmiş kosinüs zarf (tık imkânsız).
  `SignaturePlayer`: `compute()` ayrı isolate (2.3M sin → UI donmaz), 250ms fade, cold-start,
  **ayarla kapatılabilir**. `_Lcg` → public `Lcg` (kopya değil, taşındı).
- **SAĞLIK İDDİASI YOK.** Jüri mevcut alarmın 528 Hz'inin psödobilim imza frekansı olduğunu ve
  rakibin (Anima) orada konumlandığını yakaladı → 523.25'e çekilmesi **ayrı iş** (§1.1 sızma yüzeyi).
- **DOĞRULAMA:** 12 sayısal test (kırpma/tık/süreksizlik/DC/determinizm/seed/attack/tilt/ZCR).
  **3 eşik ölçülerek kalibre edildi** — spec'in analitik tahminleri saptı, spec bunu uyarmıştı;
  GÜVENLİK eşikleri değişmedi. Tam süit **445 test yeşil**, analyze temiz.
- 🔥 **Sınır:** bu kod sesin DOĞRU olduğunu kanıtlar, **GÜZEL olduğunu kanıtlamaz**. Kulaklıkla
  dinleme (§1.1) kullanıcıda.
- 📌 Mobil 82→84. Bar 71.25 ≈ **71%**.

### #196 — mobil ana ekran teması: dev-menü → 3 bölgeli gerçek ekran (PR #196)

✅ **Yapıldı ve DOĞRULANDI (433 test + 2 gerçek layout bug'ı yakalandı)** — Faz 0 cila

- **Kapatılan şikayet:** denetim ana ekranı "6 özdeş ghost buton = dev menüsü" diye niteledi.
- **Yapıldı (çok-ajanlı IA paneli + jüri spec'i):** üç bölge — **Bu gece** (ekranın tek `display`
  başlığı + tek dolu buton = gece ritüelini başlat) → **Kimlik** (tek doygun gradyan hero / solgun
  davet; yükleme-hatada da davet → ekran asla boşalmaz) → **Keşfet** (ikonlu karo rafı). Ayarlar
  gövdeden AppBar ikonuna taşındı. Streak dikey karttan yatay şeride indi. Dikey ghost dizisi: 0.
- **DOĞRULAMA:** 10 test key'i + testlerin dayandığı dizgeler korundu; **433 test yeşil**, analyze
  temiz. Testler **iki gerçek bug yakaladı**: `Row(stretch)` kaydırılabilir sütunda sınırsız yükseklik
  → `IntrinsicHeight` ile sınırlandı; kimlik hero alt satırı 167px taştı → dikey dizilime çevrildi.
- 🔥 Sınır: görsel/estetik yargı kullanıcıda (emülatör/cihazda görülecek). Gradyan üstü kontrast
  scrim'i eklendi ama **oran ölçülmedi** — karanlık odada kontrol edilmeli.
- 📌 Mobil 81→82. Bar 70.45 ≈ **70%**.

### #195 — mobil onboarding: ilk açılış karşılaması + izin priming (PR #195)

✅ **Yapıldı ve DOĞRULANDI (analyze + 433 test + debug APK build)** — Faz 0 cila devam

- **Kapatılan boşluk (Anima-denetiminin bulgusu):** uygulama SOĞUK açılıyordu — kullanıcı ne olduğunu
  anlamadan ekranla, ve uyku modunda HABERSİZ bir mikrofon izniyle karşılaşıyordu. Onboarding yoktu.
- **Yapıldı:** 3 sayfalık karşılama — **kimlik** (ücretsiz arketip testi, hesap gerekmez) → **ritüel**
  (jeneratif sesleri kendin miksle, internetsiz çalışır) → **izin priming** (mikrofonu NEDEN istiyoruz +
  **ham ses telefondan çıkmaz**, §6 vaadi açıkça yazılı). Arketip gradyanlı küreler, sayfa noktaları,
  her an **Atla** (kullanıcıyı akışta hapsetmez).
- **Mimari:** ilk-açılış kapısı `NoctaApp`'te — **router'a DOKUNULMADI** (redirect riski yok). Kalıcı
  sürümlü flag (`onboarding_seen_v1`) secure storage'da; oturum bootstrap'i onboarding okunurken PARALEL
  ilerler. Flag okunamazsa akış ATLANIR (onboarding uygulamayı asla kilitlemez).
- **DOĞRULAMA:** 5 yeni test (store kalıcılığı + ekran ilerleme/son-sayfa-bitirme + Atla + gizlilik-vaadi
  metni + kapı) → **433 test yeşil** (428→433), l10n EN/TR paritesi korundu, analyze temiz,
  **`flutter build apk --debug` BAŞARILI**. Mevcut 3 NoctaApp testine "onboarding görüldü" override'ı
  eklendi (kapı onları öne geçmesin) — kırılan 7 test böyle düzeltildi.
- 🔥 Sınır: priming yalnızca AÇIKLAR; gerçek izin istemi kendi bağlamında (uyku modu başlarken) sorulur —
  bilinçli. Görsel/metin cazibesi senin yargın (cihazda gör).
- 📌 Mobil 80→81. Kalan Faz 0: ana ekranı gerçek ekrana, Inter font, AA kontrast, hata/boş ekranlar.
  Bar 70.05 ≈ **70%**.
...
✅ **Yapıldı ve DOĞRULANDI (gerçek Redis + üretim yolu + tam süit)** — müdür (C) 2. yarı MIGRATION-GATED → mimari sertleştirme

- **Neden bu iş:** müdür (C)'nin 2. yarısı = outbox, ama outbox doğası gereği **yeni `outbox` tablosu**
  ister → §8 migration-gate (sormadan yapılmaz). Doğruladım: reuse edilebilir events/outbox tablosu
  YOK (`analytics_events` farklı amaç). Loop non-stop olmalı (soru sorup bloklamak "durma") → migration-
  gate'i rapora taşıdım, migration GEREKTİRMEYEN gerçek B4 sertleştirmeyi yaptım. main.ts'te body-limit
  ZATEN var (make-work olurdu) → kalan tek gerçek dilim: kodda ertelenmiş "Redis tabanlı dağıtık limit B4".
- **Yapıldı:** `RedisThrottlerStorage` (ThrottlerStorage impl) — atomik Lua fixed-window (INCR + ilk
  vuruşta PEXPIRE), cache #157 REDIS_URL-gate deseni (yoksa in-memory). app.module.ts: REDIS_URL varsa
  Redis depo. **Neden güvenlik:** login brute-force limiti in-memory'de her instance'ta AYRI sayılıyordu
  → çok-instance (Dockerfile #151) = N kat deneme. Redis tek sayar. Birimler varsayılan depoyla eşleşti
  (ttl ms girdi, timeToExpire saniye çıktı — kaynak okundu).
- **DOĞRULAMA:** (1) **gerçek Redis entegrasyonu** — limit-aşımı blok + IP izolasyonu + pencere sıfırlama.
  (2) in-memory e2e regresyonsuz (throttler + admin-login-throttle). (3) **üretim yolu** — REDIS_URL set
  iken app Redis depoyla açılır, "limit aşılınca 429" GERÇEK guard'dan geçer, temiz kapanır (detectOpenHandles,
  sızıntı yok). **Tam api süiti 79 suite/483 test + coverage tuttu.** typecheck+lint+build 0. CI redis
  servisi + REDIS_TEST_URL #190'da eklenmişti → CI değişikliği gerekmedi.
- 📌 **OTONOM-STOK ENVANTERİ (müdür ön-uyarısı):** bu iş müdürün öngördüğü sınırı doğruluyor — otonom
  kullanıcı-değer stoğu büyük ölçüde tükendi. Kalan **otonom** iş: outbox (MIGRATION-GATED, kullanıcı onayı),
  admin total-users metriği (küçük cila), hreflang (lansman-öncesi son web işi). Kalan **her şey insan/onay-
  kapılı**: outbox migration (§8), native ses default (kulak), iOS (Mac), IAP (Apple $99), VPS/Sentry/SMTP
  (D-serisi), gerçek gece testi, App Store, canlı domain. **"Otonom faz bitiyor, lansman kapısı insanda" —
  başarısızlık değil, sinyal (müdür).** Backend 76→77 (+1, sertleştirme). Bar 68.35 ≈ **68%**.
...
✅ **Yapıldı ve doğrulandı (4 e2e)** — müdür (C) admin'i bitir: identity A0 (rule #11 açtı)

- **Kapatılan boşluk (`enroll-totp.usecase.ts:33` notluydu):** onaylı 2FA'nın üstüne yazılamıyordu
  (`TotpAlreadyEnabledError`) → kullanıcı yeni telefona geçince 2FA'yı rotasyonlayamıyordu; kod
  aynen "parola doğrulaması isteyen ayrı bir akış olmalı (henüz yok)" diyordu.
- **Yapıldı:** `POST /v1/auth/admin/totp/reset` — PAROLA doğrulamasıyla (login ile aynı argon2)
  2FA'yı kaldırır → yeniden kurulabilir. `ResetTotpUseCase` + `UserRepository.clearTotp` (Prisma +
  in-memory). AuthGuard + login throttle (kaba kuvvet: parola tahmini 5/dk). `sub` token'dan →
  başkasının 2FA'sı sıfırlanamaz. Parola olmadan reset, oturumu ele geçirenin 2FA'yı devralmasına
  izin verirdi — o yüzden parola ZORUNLU.
- **DOĞRULAMA:** 4 e2e (gerçek Postgres): doğru parola → 2FA kalkar + yeniden kurulabilir (enroll
  409 değil 200), yanlış parola 401 + 2FA KORUNUR, auth yok 401, boş parola 400. Tam api süit
  **76 suite/475 test yeşil**, typecheck+lint temiz (boundary dahil).
- 🔥 Sınır: bu, "giriş yapılı, cihaz rotasyonu" senaryosu. "Authenticator KAYBOLDU → giriş yapamıyor"
  senaryosu ayrı (e-posta kurtarma = SMTP-gated, ya da backup-codes — ayrı iş). Reset UI (panel
  butonu) sırada. Admin 44→45 (+1). Bar 67.0 ≈ **67%**.
...
✅ **Yapıldı ve doğrulandı** — kampanya özelliği TAM (API #183 + UI #184), müdür (C) admin'i bitir

- **Yapıldı:** `/campaigns` sunucu sayfası owner'a besteleme formu gösterir: başlık + gövde
  (textarea) + hedef (tümü/iOS/Android) → Server Action `apiPost('/v1/admin/campaigns')` (#183) →
  sonuç (recipients/sent/failed). `canSendCampaigns` owner-only (API ile ayna), `campaignErrorMessage`
  ayırt edici. AppShell nav'ında Campaigns linklendi. Reach UI'da açık ("tüm segment, opt-out hariç").
- **DOĞRULAMA:** `canSendCampaigns`+`campaignErrorMessage` saf+test'li (7 unit). admin typecheck 0,
  lint 0, **vitest 20 dosya/128 test yeşil**, **Next prod build ✓** (`/campaigns` route + client form).
- 🔥 Sınır yok bu iş için — flags deseninin (API→UI) aynısı; kampanya artık uçtan uca: owner panelde
  besteleyip gönderiyor, API segmenti fan-out ediyor (gerçek APNs/FCM teslimi anahtar-gated, docs/10).
- 📌 Admin 41→43 (+2; kampanya UI yarısı). Kalan admin: D7/metrik (veri-gated) + 2FA/davet/parola-
  sıfırlama (rule #11 otonom). Bar 66.7 ≈ **67%**.
...
✅ **Yapıldı ve DOĞRULANDI (tarayıcıda)** — müdür hükmü (A): web en büyük otonom kaldıraç

- **Müdür beni §0.6 kapsam-daraltmada YAKALADI:** web'i "sadece lighthouse config (0.15)"
  diye küçülttüm, oysa tablom web kalanını "long-tail, **blog**" yazıyordu. Web %35, kalanı
  %100 otonom + kullanıcıya dönük (docs/05: web = viral ön-lansman/edinim kanalı, polish değil).
  Hüküm: (A) otonom faz BİTMEDİ, web'e kürek salla.
- **Yapıldı:** dosya-tabanlı blog pipeline (`content/blog.ts` — TS obje, MDX değil: `archetypes.ts`
  deseniyle tutarlı, yeni bağımlılık yok §8, tip-güvenli, test'li). 3 seed yazı (wind-down ritual,
  soundscapes-explained, what-is-a-sleep-identity — gerçek nesir, ürüne iç-bağlantılı, /test'e CTA).
  `/blog` dizin + `/blog/[slug]` SSG sayfaları + `buildBlogPostJsonLd` (Article) + Breadcrumb +
  sitemap wiring. **4 yeni indekslenebilir sayfa.**
- **DOĞRULAMA:** `check:health-claims` GEÇTİ (397 dosya, iddia yok — §1.1 kapısı). 8 test (blog
  yapı+sağlık-iddiası+sıra, routes sitemap). typecheck+lint temiz, Next build (22 statik sayfa),
  bundle 107≤115kB. **TARAYICIDA:** /blog 3 yazı listeler, /blog/wind-down-ritual başlık +
  `"@type":"Article"` + datePublished + BreadcrumbList, sitemap.xml 4 blog rotası içeriyor.
- 📌 Kararlar (müdür bana bıraktı): TS-obje (MDX değil), 3 seed yazı, konu=uyku ritüeli rehberleri
  (sağlık iddiası YASAK — hepsi "relaxation & ritual"). Sıradaki web dilimi: hreflang EN/TR
  (mevcut 4 arketibi ikiye katlar) + daha fazla yazı, sonra CWV lighthouse-ci. Web 35→38 (+3).
  Bar 65.35 ≈ **65%**.
...
✅ **Yapıldı ve DOĞRULANDI (emülatörde derlendi + ateşlendi + OS'a kayıtlı)**

- **#169'un ertelenen native tail'i:** #169 Dart dikişini kurmuştu ama AlarmManager
  handler'ı "cihaz-kapılı" diye ertelenmişti (backstop fiilen çalışmıyordu, sessiz no-op).
  Emülatör-Kotlin-doğrulama yeteneği (#172/#173) bunu OTONOM yaptı → tamamlandı.
- **Yapıldı:** `NightAlarm` (Kotlin) `setAlarmClock` ile son-tarihi OS'a kaydeder
  (kesin + Doze-proof + gerçek uyandırma-alarmı semantiği); `NightAlarmReceiver`
  (manifest-kayıtlı) ateşlemede tam-ekran (USE_FULL_SCREEN_INTENT) uyandırma bildirimi
  çıkarır. MainActivity `nocta/night_alarm` kanalı (#169 sözleşmesi). Manifest receiver.
- **DOĞRULAMA (kanıtlı):** (1) `flutter build apk` → Kotlin DERLENDİ; (2) integration_test
  emülatör-5554'te GEÇTİ (schedule/cancel e2e); (3) **logcat: `scheduled`→`fired`→`cancelled`
  — alarm FİİLEN ATEŞLENDİ, receiver çalıştı**; (4) **dumpsys alarm: `type=RTC_WAKEUP
tag=*walarm*:com.nocta.nocta/.NightAlarmReceiver`** — alarm OS AlarmManager'a kayıtlı
  (süreçten bağımsız). Tam mobil süit 411 yeşil, analyze temiz.
- 🔥 **Sınır (dürüstlük):** schedule+ateşleme+cancel+OS-kayıt KANITLI → backstop artık
  çalışıyor. Ama **literal "app öldürülünce ateşleme"** emülatörde temiz gösterilemedi:
  `am force-stop` Android tasarımı gereği alarmı İPTAL eder; `am kill` emülatör
  App-Standby'ı yüzünden pencerede ateşlemedi. `setAlarmClock`+manifest-receiver+RTC_WAKEUP
  MİMARİSİ süreç-ölümünde ateşleme İÇİN tasarlanmıştır (OS-sahipli, dumpsys'te kanıtlı) ama
  bu spesifik kenar GERÇEK CİHAZ doğrulaması ister. #169'a göre dev ilerleme: no-op → çalışır.
- 📌 Mobil 75→76: cihazda ateşleme+kayıt doğrulanmış gerçek yetenek tamamlandı; +1 ile
  sınırlı (dead-process kenarı gerçek cihaz bekler).
